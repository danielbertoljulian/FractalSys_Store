...
export const LaserFlow: React.FC<LaserFlowProps> = ({
  ...
  const hexToRGB = (hex: string) => {
    const safeHex = typeof hex === 'string' && hex.length > 0 ? hex : '#FFFFFF';
    let c = safeHex.trim();
    if (c[0] === '#') c = c.slice(1);
    if (c.length === 3)
      c = c
  ...
  useEffect(() => {
    ...
    const clampAux = (v: number, lo: number, hi: number) => Math.max(lo, Math.min(hi, v));
    const dprFloor = 0.6;
    ...
    const adjustDprIfNeeded = (now: number) => {
      ...
      if (avgFps < lowerThresh) {
        next = clampAux(currentDprRef.current * 0.85, dprFloor, base);
      } else if (avgFps > upperThresh && currentDprRef.current < base) {
        next = clampAux(currentDprRef.current * 1.1, dprFloor, base);
      }
